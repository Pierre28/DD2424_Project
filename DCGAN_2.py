...
class DCGAN():
    def __init__(self, input_shape, first_block_depth=1024, dim_noise=100):
        ...
        self.X_batch = tf.placeholder(dtype=tf.float32, shape=[None, self.output_side*self.output_side], name='X') #Laisser None pour le batch ?
    ...
    def initialize_variables(self):
        # Variables
        #self.get_noise(1)
        #self.X_batch = tf.random_uniform([100, 784], -1, 1) #Used for initialize true image ? Shape does not correspond to the shape from placeholder
        ...
        ini_proba_real, ini_logits_real = self.discriminator.forward_pass(self.X_batch)
        # Loss
    ...
    def train(self, X, n_epochs, batch_size):
        ...
        #X = np.zeros((1000, 28, 28, 1), dtype = 'float32')
        ...
        with tf.Session() as sess:
            ...
            for i in range(n_epochs):                
                ...
                for j in range(1, max_j):
                    ...
                    j_end = j * batch_size
                    self.X_batch_ = X[j_start:j_end].reshape((100, 784))
